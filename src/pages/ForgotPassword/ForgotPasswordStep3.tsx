...
export function ForgotPasswordStep3({ setStep }: ForgotPasswordStep3Props) {
  const [password, setPassword] = useState('');
  const [confirmPassword, setConfirmPassword] = useState('');
  const { validateField, errors } = useFormValidation();

  const handleResetPassword = () => {
    if (validateField('password', password) && validateField('confirmPassword', confirmPassword, password)) {
      setStep(4);
    }
  };
  ...
  return (
    ...
    >
      ...
      <motion.div className="w-[400px]" variants={fadeIn} initial="initial" animate="animate">
      ...
      <motion.div variants={fadeIn} initial="initial" animate="animate">
        <Button className="mt-[24px] w-[400px] h-[80px] bg-[#5F24E0] hover:bg-[#9F7CEC] text-[#EFE9FC] font-['Archivo'] text-[22px] font-semibold rounded-[24px]" onClick={handleResetPassword}>
          Reset Password
        </Button>
      </motion.div>
    </motion.div>
